...
df.combo <- df.SYNOP %>%
  ...
  select(SYNOP.dCFC.mu, SYNOP.dCFC.se, val.signif, month) %>%
  left_join(df.MODIS, by = 'month')

df.combo2 <- left_join(
  df.combo %>% 
    select(month, MODIS.dCFC.mu, SYNOP.dCFC.mu) %>%
    pivot_longer(cols = c('MODIS.dCFC.mu','SYNOP.dCFC.mu'), 
                 names_to = 'source', 
                 names_pattern = "(.*).dCFC.mu",
                 values_to = 'dCFC.mu'),
  df.combo %>% 
    select(month, MODIS.dCFC.se, SYNOP.dCFC.se) %>%
