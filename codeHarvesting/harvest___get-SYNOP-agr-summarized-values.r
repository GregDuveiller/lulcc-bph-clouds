# Harvest the SYNOP data to make the plots for a given region 
...
save('df_SYNOP_agr', 'df_SYNOP_loc',
     file = paste0('dataFigures/df_SYNOP_agr_4polarplots_',regio.name,'.RData'))
...
# 
# ggplot(df.all %>% 
#          filter(thr.dfor.min == .2, thr.nyrs.min == 7), 
#        aes(x = hour, y = month)) +
...
# 
# ggplot(df.all %>% 
#          filter(thr.dist.min == 30, thr.dist.max == 60)) +
#   aes(x = hour, y = month) +
#   geom_tile(aes(fill = dCFC)) +
#   geom_point(aes(alpha = val.signif)) +
#   scale_alpha_manual(values = c(0,1), guide = F) +
...
#                        limits = c(-0.08,0.08), oob = scales::squish) +
#   coord_polar() +
#   facet_grid(thr.dfor.min~thr.nyrs.min) +
#   theme(legend.position = 'bottom',
#         legend.key.width = unit(2.4, "cm"),
#         panel.background = element_rect(fill = 'white'),
#         axis.title = element_blank(),
