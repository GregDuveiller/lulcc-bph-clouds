...
synop.g = readOGR("data/inter_data/synop_analysis", "SYNOPpoints_withAncillaryData")
...
for(i in 1:dim(crds)[1])
{
  ...
  if(length(w_)>0){
    ...
    for(j in 1:length(w_))
    {
      ...
      p.1 = rbind(p.1, data.frame(id= synop$sitename[synop$lon==crds[i,1]&synop$lat==crds[i,2]], lon=crds[i,1],lat=crds[i,2],area=synop.g.1$lcl_tr_[i], dist=dists_[w_[j]]))
      p.2 = rbind(p.2, data.frame(id= synop$sitename[synop$lon==crds[w_[j],1]&synop$lat==crds[w_[j],2]], lon=crds[w_[j],1],lat=crds[w_[j],2],area=synop.g.1$lcl_tr_[w_[j]], dist=dists_[w_[j]]))
    }
  }
}
    ...
    if(dim(p1)[1] >0)
    {
      for(month in 1:12) {
        for(hour in 0:23) {
          ...
          if(length(which_) > 0)
            paired = rbind(paired, data.frame(pair=i, month=month, hour=hour, area.h=area.h, area.l=area.l, dist=p.1$dist[i], diff=mean(p1[which_,H]-p1[which_,L]) / 8 * 100, N=length(which_), lon=(p.1$lon[i]+p.2$lon[i])*0.5, lat=(p.1$lat[i]+p.2$lat[i])*0.5))
        }
      }
    }    
