...
################################################################################
# Purpose: Aggregate and harvest necessary data from the S4T datasets
# License: GPL v3
...
# MOD05 S4T result ----
S4T_file2harvest <- paste0(S4T_path, '/ESACCI-L3X_CLOUD-JRC-MODIS_AQUA-PM-fv2.0_cfc_all_2004_2014_monthly_avg_climatology_topography_masked_s4t_winsize7_s4t_subset_masked_aggregated_0.35.nc')
...
save('df_dCFC_MOD05_DFO', file = paste0(harvest_path, '/df_dCFC_MOD05_DFO.Rdata'))
...
df_dCFC_MOD05_DFO_1dd <- as.data.frame(rs_DFO_delta_1dd, xy = T, long = T) %>% 
  ...
  dplyr::select(-layer) %>%
  dplyr::filter(!is.na(dCFC))
save('df_dCFC_MOD05_DFO_1dd', file = paste0(harvest_path, '/df_dCFC_MOD05_DFO_1dd.Rdata'))
...
df_dCFC_MOD05_EFO <- as.data.frame(rs_EFO_delta, xy = T, long = T) %>% 
  ...
  dplyr::select(-Z) %>%
  dplyr::filter(!is.na(dCFC))
save('df_dCFC_MOD05_EFO', file = paste0(harvest_path, '/df_dCFC_MOD05_EFO.Rdata'))
...
df_dCFC_MOD05_EFO_1dd <- as.data.frame(rs_EFO_delta_1dd, xy = T, long = T) %>% 
  ...
  dplyr::select(-layer) %>%
  dplyr::filter(!is.na(dCFC))
save('df_dCFC_MOD05_EFO_1dd', file = paste0(harvest_path, '/df_dCFC_MOD05_EFO_1dd.Rdata'))
...
# export raster
writeRaster(x = rs_FOR_delta, 
            filename = paste0(harvest_path, "/rstr_dCFC_MOD05_FOR.nc"), 
...
df_dCFC_MOD05_FOR <- as.data.frame(rs_FOR_delta, xy = T, long = T) %>% 
  ...
  dplyr::select(-layer) %>%
  dplyr::filter(!is.na(dCFC))
save('df_dCFC_MOD05_FOR', file = paste0(harvest_path, '/df_dCFC_MOD05_FOR.Rdata'))
...
df_dCFC_MOD05_FOR_1dd <- as.data.frame(rs_FOR_delta_1dd, xy = T, long = T) %>% 
  ...
  dplyr::select(-layer) %>%
  dplyr::filter(!is.na(dCFC))
save('df_dCFC_MOD05_FOR_1dd', file = paste0(harvest_path, '/df_dCFC_MOD05_FOR_1dd.Rdata'))
...
# load S4T dataset
S4T_file2harvest <- paste0(S4T_path, '/ESACCI-L3U_CLOUD-CLD_PRODUCTS-MODIS_AQUA_Europe-fv2.0_cmask_2004_2014_monthly_avg_climatology_clip_masked_winsize7_0.02dd_s4t_subset_masked_aggregated_0.14deg.nc')
...
df_dCFC_MOD02_DFO <- as.data.frame(rs_DFO_delta, xy = T, long = T) %>% 
  ...
  dplyr::select(-Z) %>%
  dplyr::filter(!is.na(dCFC))
save('df_dCFC_MOD02_DFO', file = paste0(harvest_path, '/df_dCFC_MOD02_DFO.Rdata'))
...
df_dCFC_MOD02_DFO_1dd <- as.data.frame(rs_DFO_delta_1dd, xy = T, long = T) %>% 
  ...
  dplyr::select(-layer) %>%
  dplyr::filter(!is.na(dCFC))
save('df_dCFC_MOD02_DFO_1dd', file = paste0(harvest_path, '/df_dCFC_MOD02_DFO_1dd.Rdata'))
...
df_dCFC_MOD02_EFO <- as.data.frame(rs_EFO_delta, xy = T, long = T) %>% 
  ...
  dplyr::select(-Z) %>%
  dplyr::filter(!is.na(dCFC))
save('df_dCFC_MOD02_EFO', file = paste0(harvest_path, '/df_dCFC_MOD02_EFO.Rdata'))
...
df_dCFC_MOD02_EFO_1dd <- as.data.frame(rs_EFO_delta_1dd, xy = T, long = T) %>% 
  ...
  dplyr::select(-layer) %>%
  dplyr::filter(!is.na(dCFC))
save('df_dCFC_MOD02_EFO_1dd', file = paste0(harvest_path, '/df_dCFC_MOD02_EFO_1dd.Rdata'))
...
# export raster
writeRaster(x = rs_FOR_delta, 
            filename = paste0(harvest_path, "/rstr_dCFC_MOD02_FOR.nc"), 
            format = "CDF", overwrite = TRUE)
...
df_dCFC_MOD02_FOR <- as.data.frame(rs_FOR_delta, xy = T, long = T) %>% 
  ...
  dplyr::select(-layer) %>%
  dplyr::filter(!is.na(dCFC))
save('df_dCFC_MOD02_FOR', file = paste0(harvest_path, '/df_dCFC_MOD02_FOR.Rdata'))
...
df_dCFC_MOD02_FOR_agr <- as.data.frame(rs_FOR_delta_agr, xy = T, long = T) %>% 
  ...
  dplyr::select(-layer) %>%
  dplyr::filter(!is.na(dCFC))
save('df_dCFC_MOD02_FOR_agr', file = paste0(harvest_path, '/df_dCFC_MOD02_FOR_agr.Rdata'))
...
df_dCFC_MOD02_FOR_1dd <- as.data.frame(rs_FOR_delta_1dd, xy = T, long = T) %>% 
  ...
  dplyr::select(-layer) %>%
  dplyr::filter(!is.na(dCFC))
save('df_dCFC_MOD02_FOR_1dd', file = paste0(harvest_path, '/df_dCFC_MOD02_FOR_1dd.Rdata'))
...
# load S4T dataset
S4T_file2harvest <- paste0(S4T_path, '/ESACCI-L3U_CLOUD-CLD_PRODUCTS-MODIS_AQUA_Europe-fv2.0_cmask_2004_2014_monthly_avg_climatology_clip_masked_winsize17_0.02dd_s4t_subset_masked_aggregated_0.34deg.nc')
...
df_dCFC_MODo2_DFO <- as.data.frame(rs_DFO_delta, xy = T, long = T) %>% 
  ...
  dplyr::select(-Z) %>%
  dplyr::filter(!is.na(dCFC))
save('df_dCFC_MODo2_DFO', file = paste0(harvest_path, '/df_dCFC_MODo2_DFO.Rdata'))
...
df_dCFC_MODo2_DFO_1dd <- as.data.frame(rs_DFO_delta_1dd, xy = T, long = T) %>% 
  ...
  dplyr::select(-layer) %>%
  dplyr::filter(!is.na(dCFC))
save('df_dCFC_MODo2_DFO_1dd', file = paste0(harvest_path, '/df_dCFC_MODo2_DFO_1dd.Rdata'))
...
df_dCFC_MODo2_EFO <- as.data.frame(rs_EFO_delta, xy = T, long = T) %>% 
  ...
  dplyr::select(-Z) %>%
  dplyr::filter(!is.na(dCFC))
save('df_dCFC_MODo2_EFO', file = paste0(harvest_path, '/df_dCFC_MODo2_EFO.Rdata'))
...
df_dCFC_MODo2_EFO_1dd <- as.data.frame(rs_EFO_delta_1dd, xy = T, long = T) %>% 
  ...
  dplyr::select(-layer) %>%
  dplyr::filter(!is.na(dCFC))
save('df_dCFC_MODo2_EFO_1dd', file = paste0(harvest_path, '/df_dCFC_MODo2_EFO_1dd.Rdata'))
...
df_dCFC_MODo2_FOR <- as.data.frame(rs_FOR_delta, xy = T, long = T) %>% 
  ...
  dplyr::select(-layer) %>%
  dplyr::filter(!is.na(dCFC))
save('df_dCFC_MODo2_FOR', file = paste0(harvest_path, '/df_dCFC_MODo2_FOR.Rdata'))
...
df_dCFC_MODo2_FOR_1dd <- as.data.frame(rs_FOR_delta_1dd, xy = T, long = T) %>% 
  ...
  dplyr::select(-layer) %>%
  dplyr::filter(!is.na(dCFC))
save('df_dCFC_MODo2_FOR_1dd', file = paste0(harvest_path, '/df_dCFC_MODo2_FOR_1dd.Rdata'))
